...
echo
echo -e "\e[1;4mWhat should irc equal? (no freq calc)" 
echo -e "\e[0;36m[1] (calcfc,forward,maxstep=150,maxpoint=150)"
echo -e "\e[0;36m[2] (calcfc,reverse,maxstep=150,maxpoint=150)"
echo -e "\e[92m[3] (rcfc,reverse,maxstep=20,maxpoint=100) Guess=Read Geom=Checkpoint (REMOVES XYZ!)"
echo -e "\e[92m[4] (rcfc,forward,maxstep=20,maxpoint=100) Guess=Read Geom=Checkpoint (REMOVES XYZ!)"
echo -e "\e[33m[5] restart,maxcyc=512"
echo -e "\e[35m[6] manual input \e[0m"
...
# Note variable is quoted.
  ...
  "1" | "1" )
      w_new="(calcfc,forward,maxstep=20,maxpoint=100)"
...
# Note double semicolon to terminate each option.
 ...
 "2" | "2" )
      w_new="(calcfc,reverse,maxstep=20,maxpoint=100)"
	#will set w_new variable to the answer choice, the other "1" is redundant, leftover from allowing capital or lowercase letters
  ...
  "3" | "3" )
    w_new="(rcfc,reverse,maxstep=20,maxpoint=100) Guess=Read Geom=Checkpoint"
	w_new2="(rcfc,reverse,maxstep=20,maxpoint=100) Guess=Read Geom=Checkpoint"
	sed '11,10000d' $input 
	
  ;;

  "4" | "4" )
    w_new="(rcfc,forward,maxstep=20,maxpoint=100) Guess=Read Geom=Checkpoint"
	w_new2="(rcfc,forward,maxstep=20,maxpoint=100) Guess=Read Geom=Checkpoint"
	sed '11,10000d' $input
...
echo -e "\e[0;36m[Y or 1] yes"
echo -e "\e[92m[N or 2] no\e[0m"
...
%mem=$w_mem
%chk=$name.chk
# irc=$w_new $w_fun scrf=(smd,solvent=$w_sol)
...
tail -n +11 $input >> $name""new.com #grabs xyz minus first two lines
 if [ "$w_new" = "$w_new2" ]; then 
 sed -i '11,10000d' $name""new.com
 fi
 if [ "$w_new" = "$w_new2" ];
 then
 sed -i '11,10000d' $name""new.com 
